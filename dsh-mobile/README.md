...
一个 App 可以保存多台 DSH 设备（Mac mini / Windows / Linux 开发机等），
每台设备独立保存「连接模式 + 服务器地址 + 访问凭据」，互不覆盖。

- 设置页「当前设备」卡片进入**设备管理**：查看全部设备、点击切换、
  添加新设备、删除不再使用的设备；
- 切换设备后 Flutter 主页会立即用新设备的地址重建 WebView，
  WebSocket/会话状态完全切换到目标设备；
- 凭据仍按设备分别存放在 Android Keystore，列表里只存设备名称、
  模式与服务器 origin，绝不明文保存口令；
- 旧版单设备配置会自动迁移成一条「默认设备」，升级无感。
