...
> 版本对应：本套件当前对应官方 **DeepSeek Harness 0.1.1-rc.2**。
> 服务端各设备请升级到 0.1.1-rc.2；移动 Web 壳按该版本官方启动协议构建，不再依赖 rc.8 版后构建补丁。
...
- **多设备管理**：一个 App 保存多台 DSH 设备（Windows/macOS/Linux），
  设备管理页一键切换，凭据按设备独立加密存储；
...
- 基于 DSH 0.1.1-rc.2 源码构建移动 Web 壳，不再依赖 rc.5-era dist 的后构建补丁。
