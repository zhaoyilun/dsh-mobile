...
async function loadComposition(mountPath?: string): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    '  config:',
    `    distIndex: '${distIndex}'`,
    ...(mountPath === undefined ? [] : [`    path: '${mountPath}'`]),
    '',
  ].join('\n'))
...
describe('real Loader composition', () => {
  ...
  it('serves the dist under a prefix route with the same static semantics and a free fallback seat', { timeout: 60_000 }, async () => {
    const loaded = await loadComposition('/m')
    const server = loaded.webServer
    const port = server.port

    // Real assets under the prefix, with their MIME types.
    expect(await request(port, '/m/app.js')).toMatchObject({ status: 200, type: 'text/javascript; charset=utf-8', body: 'export {}' })
    expect(await request(port, '/m/blob.bin')).toMatchObject({ status: 200, type: 'application/octet-stream', body: 'BLOB' })

    // The prefix root, the index path, and any miss under the prefix all
    // render index.html (SPA routing) through the registered index taps.
    const untap = server.tapIndex(html => html.replace('<head>', '<head><script>window.__T__=1</script>'))
    for (const path of ['/m', '/m/', '/m/index.html', '/m/no/such/route']) {
      const got = await request(port, path)
      expect(got.status).toBe(200)
      expect(got.body).toContain('__T__')
      expect(got.body).toContain('shell')
    ...
    untap()
    expect((await request(port, '/m/')).body).not.toContain('__T__')

    // Traversal outside the dist root is 403 under the prefix; non-GET/HEAD
    // is 405 under the prefix.
    expect((await request(port, '/m/..%2f..%2fetc%2fpasswd')).status).toBe(403)
    expect((await request(port, '/m/nowhere', { method: 'POST' })).status).toBe(405)

    // The fallback seat stays free in path mode: a non-prefix path falls
    // through to the webserver's 404.
    expect((await request(port, '/')).status).toBe(404)

    // HMR safety: disposing the frontend row removes the prefix route (the
    // path falls back to the webserver 404) and the route is claimable again.
    const frontendEntry = [...loaded.loader.entries()].find(e => e.options.id === 'frontend')
    expect(frontendEntry).toBeDefined()
    await frontendEntry!.fiber?.dispose()
    expect((await request(port, '/m/nowhere')).status).toBe(404)
    expect(() => server.register({ kind: 'prefix', path: '/m', handler: () => {} })).not.toThrow()
    // The fallback seat itself was never claimed by the path-mode row.
    expect(() => server.registerFallback(() => {})).not.toThrow()
  })
})
