...
describe('real Loader composition', () => {
  ...
  it('runs the request guard before HTTP and upgrade dispatch, with replacement and disposal', { timeout: 60_000 }, async () => {
    const loaded = await loadComposition()
    const server = loaded.webServer
    const port = server.port
    server.register({ kind: 'exact', path: '/guarded', handler: (_req, res) => { res.writeHead(200); res.end('GUARDED') } })

    // A rejecting guard answers the request itself (here a 401); the route
    // never runs.
    const releaseRejecting = server.setRequestGuard((_req, res) => {
      if (res !== undefined) {
    ...
    // Repeated registration replaces the previous gate (newest wins).
    server.setRequestGuard(() => true)
    expect(await request(port, '/guarded')).toMatchObject({ status: 200, body: 'GUARDED' })

    // An async gate is supported (and, like any rejecting HTTP gate, writes
    // its own rejection response).
    server.setRequestGuard(async (_req, res) => {
      await Promise.resolve()
      if (res !== undefined) {
        res.writeHead(401, { 'content-type': 'text/plain; charset=utf-8' })
        res.end('denied')
    ...
    // The disposer removes exactly the gate it registered; without any gate
    // installed the route serves as before (no authentication).
    const releaseAdmitting = server.setRequestGuard(() => true)
    expect(await request(port, '/guarded')).toMatchObject({ status: 200, body: 'GUARDED' })
    releaseAdmitting()
    expect(await request(port, '/guarded')).toMatchObject({ status: 200, body: 'GUARDED' })

    // Upgrades pass through the same gate: a rejection destroys the socket
    // without a handshake, admission reaches the upgrade route.
    server.registerUpgrade({
      path: '/sock',
      handler: (_req, socket) => {
        socket.write('HTTP/1.1 101 Switching Protocols\r\nConnection: Upgrade\r\nUpgrade: dsh-test\r\n\r\n')
    ...
    server.setRequestGuard(() => false)
    const rejected = connect(port, '127.0.0.1')
    rejected.on('error', () => { /* The server-side reset is the fixture outcome. */ })
    ...
    await rejectedClosed // destroyed without the 101 handshake
    server.setRequestGuard(() => true)
    const accepted = await upgrade(port, '/sock')
    accepted.destroy()

    // The rejecting guard's disposer is a no-op after replacement.
    releaseRejecting()
    expect(await request(port, '/guarded')).toMatchObject({ status: 200, body: 'GUARDED' })
