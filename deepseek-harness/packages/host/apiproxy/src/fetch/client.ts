...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // crypto.randomUUID needs a secure context (https or localhost): plain-HTTP
    // LAN serving reaches the browser without one, so fall back to a
    // getRandomValues-based v4 UUID — available on insecure origins in every
    // browser — keeping this base platform-neutral either way.
