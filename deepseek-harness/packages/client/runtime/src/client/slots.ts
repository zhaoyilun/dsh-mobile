...
export class SlotRegistry extends Service {
  ...
  /**
   * The single ctx-level render entry: the shell renders its configured root
   * slot (the built-in 'root', or a second shell's declared root key such as
   * 'mobile-frame'); every other key renders inside components through the
   * props renderSlot face. All three guards are fail-loud boot-order checks,
   * no fallback.
   * @param key - the renderer's configured root slot key (the runtime enforces
   * this for dynamically composed callers).
  ...
  renderSlot<K extends keyof SlotMap & string>(key: K, owner: OwnerOf<K>): ReturnType<SlotRenderer['renderRoot']> {
    ...
    // and cross-program callers where K is wider. The renderer's rootKey
    // (default 'root') is the shell's declared render entry; the desktop
    // program's renderer never sets it, so its behavior is unchanged.
    const rootKey = this._renderer?.rootKey ?? 'root'
    if ((key as string) !== rootKey) {
      throw new Error(`ctx-level renderSlot only renders '${rootKey}' (got "${key}"); child slots render through the component props face`)
    }
    ...
    if (this._core.entries(rootKey).length === 0) {
      throw new Error(`'${rootKey}' has no registration — a layout entry must register into '${rootKey}' before the shell renders it`)
    }
