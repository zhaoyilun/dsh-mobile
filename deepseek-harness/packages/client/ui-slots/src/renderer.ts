...
export interface SlotRenderer {
   ...
   * @param ownerProps - owner props from the shell's renderSlot(..., ...) call.
   * @returns the rendered tree.
   */
  renderRoot(host: SlotRendererHost, ownerProps: object): ReactNode
  /**
   * The shell-configured root slot key {@link SlotRenderer.renderRoot}
   * dispatches; absent (or 'root') renders the built-in 'root' slot. The
   * runtime SlotRegistry validates shell renderSlot calls against this key, so
   * a second shell (mobile) can render its own declared root slot through the
   * same machinery while the desktop 'root' content stays inert.
   */
