...
describe('connection client apply', () => {
  ...
  it('opens one WebSocket per downlink, parses frames, and aborts both after a ticket mint each', async () => {
    ...
    // The ticket mint is the one fetch per downlink; it fails closed here
    // (spy resolves nothing) and the handshake proceeds unticketed.
    ...
    // Two downlinks, one mint fetch each — and the minted value never
    // reached a handshake URL because every mint rejected.
    expect(fetch).toHaveBeenCalledTimes(2)
    expect(sockets.every(socket => !socket.url.includes('ticket='))).toBe(true)
