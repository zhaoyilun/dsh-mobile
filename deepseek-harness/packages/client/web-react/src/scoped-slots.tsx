...
function RootOutlet({ slotKey, ownerProps }: { slotKey: string; ownerProps: object }) {
  const host = useHost()
  useSyncExternalStore(
    fn => host.subscribe(slotKey, fn),
    () => host.getVersion(slotKey),
  ...
  if (!entry) {
    ...
    if (host.entriesOf(slotKey).length > 0) return <div data-slot-error={slotKey} />
    throw new SlotAssemblyError(`renderSlot('${slotKey}') before any '${slotKey}' registration (boot order)`)
  }
  // Same anchor contract as SlotOutlet: the root is a slot like any other, and
  ...
  return (
    <div data-slot={slotKey} style={ANCHOR_STYLE}>
      <SlotErrorBoundary
        ...
        onEntryError={(error) => { host.reportEntryError(slotKey, entry, error, { abdicate: true }) }}
      >
        <RootEntry
          entry={entry}
          ownerProps={ownerProps}
...
/** Options for {@link createSlotRenderer}. */
export interface SlotRendererOptions {
  /**
   * Root slot key the renderer dispatches in place of the built-in 'root' —
   * a second shell's declared render entry. The runtime SlotRegistry reads it
   * back from the installed renderer to authorize the shell's renderSlot
   * calls. Absent keeps the historical 'root' dispatch.
   */
  rootKey?: string
}
 ...
 * The optional `rootKey` lets a second shell (mobile) render its own declared
 * root slot through the identical machinery while the desktop 'root' content
 * stays inert — one composition, two shells.
 * @param options - renderer options; `rootKey` defaults to 'root'.
 * @returns the renderer.
 */
export function createSlotRenderer(options?: SlotRendererOptions): SlotRenderer {
      ...
      return (
        <HostContext.Provider value={host}>
          <SessionMaybeProvider>
            <RootOutlet slotKey={rootKey} ownerProps={ownerProps} />
          </SessionMaybeProvider>
...
/** Props of {@link DeclaredSlotOutlet}: a slot key plus the owner/opts the declarer would pass. */
export interface DeclaredSlotOutletProps {
  /** Declared SlotMap key whose content should render (the outlet reads the ledger directly). */
  slotKey: string
  /** Owner props share for that slot (decided at this render site). */
  ownerProps: object
  /** Kind dispatch options (keyed entryKey, list filtering, empty fallback). */
  opts?: RenderOpts
}

/**
 * Render a DECLARED slot by key without declaring it as this entry's child —
 * the second-shell escape hatch (a mobile frame embedding the desktop
 * 'conversation' column it does not own). The render site is the same
 * machinery every other outlet uses (session scope, chain election, store
 * axis, error containment); only the declaration-ownership check is skipped,
 * so a shell may compose slots another entry declared. Type safety: the key
 * is a runtime string here (the shell cannot statically narrow an
 * un-declared key); callers name declared keys of the program's SlotMap.
 * @param props - the target key, owner share, and dispatch options.
 * @returns the rendered slot content.
 */
export function DeclaredSlotOutlet({ slotKey, ownerProps, opts }: DeclaredSlotOutletProps): ReactNode {
  // Rendered inside the installed renderer tree, so the host context is
  // present; the outlet machinery (SlotOutlet) consumes it.
  return <SlotOutlet slotKey={slotKey} ownerProps={ownerProps} opts={opts} />
}
